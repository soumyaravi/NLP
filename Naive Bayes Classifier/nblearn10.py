    ...
    for subdir, dirs, files in os.walk(args[0]):
        for file in files:
            if (file.endswith('.txt')):
                ...
                infile.close()
                if file.__contains__('ham') and ham_files<=953:
                    ham_files += 1
                    for datas in words:
                        data = datas.rstrip()
                        if data in ham:
                        ...
                        if data not in distinct:
                            distinct[data] = 1
                elif file.__contains__('spam') and spam_files<=750:
                    spam_files += 1
                    for data in words:
                        if data in spam:
...
def writeOutput(spam_count,ham_count):
    with open('nbmodel2.txt', 'w', encoding="latin1") as out:
...
if __name__ == "__main__":
    if len(sys.argv)!=2:
        print('Usage: python nblearn10.py <path>')
        sys.exit(1)
